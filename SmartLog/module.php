...
class SmartLog extends IPSModuleStrict
{
    ...
    public function Create(): void
    {
        ...
        // Variablen
        $this->RegisterVariableInteger('EntryCount', 'Log-Eintraege', ['PRESENTATION' => VARIABLE_PRESENTATION_VALUE_PRESENTATION, 'ICON' => 'list'], 1);
    ...
    public function ApplyChanges(): void
    {
        ...
        if ($this->ReadPropertyBoolean('EnableSecurityLogging')) {
            ...
            if ($inventoryId > 0 && @IPS_InstanceExists($inventoryId) && function_exists('SINV_GetByCategory')) {
                // 1. Fenster/Türen (contact)
                $contacts = json_decode((string)@SINV_GetByCategory($inventoryId, 'contact'), true) ?: [];
                foreach ($contacts as $c) {
                    $varId = (int)($c['varID'] ?? 0);
                    if ($varId > 0 && @IPS_VariableExists($varId)) {
                        $securityMap[$varId] = [
                            'type'        => 'Contact',
                            'name'        => $c['instanceName'] ?? 'Fenster',
                ...
                // 2. Garagen (contact:garage)
                $garages = json_decode((string)@SINV_GetByCategory($inventoryId, 'contact:garage'), true) ?: [];
                foreach ($garages as $g) {
                    $varId = (int)($g['varID'] ?? 0);
                    if ($varId > 0 && @IPS_VariableExists($varId)) {
                        $securityMap[$varId] = [
                            'type'        => 'Garage',
                            'name'        => $g['instanceName'] ?? 'Garagentor',
                            'room'        => $g['room'] ?? 'Garage',
                            'normalState' => $g['normalState'] ?? null,
                        ];
                        $this->RegisterMessage($varId, VM_UPDATE);
                    }
                }
                // 3. Schlösser (actor:lock)
                $locks = json_decode((string)@SINV_GetByCategory($inventoryId, 'actor:lock'), true) ?: [];
                foreach ($locks as $l) {
                    $varId = (int)($l['varID'] ?? 0);
                    if ($varId > 0 && @IPS_VariableExists($varId)) {
                        $securityMap[$varId] = [
                            'type'        => 'Lock',
                            'name'        => $l['instanceName'] ?? 'Schloss',
                            'room'        => $l['room'] ?? 'Eingang',
                            'normalState' => $l['normalState'] ?? null,
                        ];
                        $this->RegisterMessage($varId, VM_UPDATE);
                    }
                }
    ...
    public function MessageSink(int $TimeStamp, int $SenderID, int $Message, array $Data): void
    {
        if ($Message == VM_UPDATE) {
            ...
            if (is_array($securityMap) && isset($securityMap[$SenderID])) {
                ...
                $val = $Data[0]; // aktueller Wert (bool/int/string)

                $msg = '';
                if ($dev['type'] === 'Contact' || $dev['type'] === 'Garage') {
